...
inline void LoadJsonValue(const rapidjson::Value& v, std::string& t) {
    if (v.IsString()) {
        ...
        throw MujinJSONException("Cannot convert json value " + GetJsonString(v) + " to String", MJE_Failed);
    }
...
template<size_t N> inline void SaveJsonValue(rapidjson::Value& v, const boost::array<double, N>& t, rapidjson::Document::AllocatorType& alloc) {
    v.SetArray();
    v.Reserve(N, alloc);
    for (size_t i = 0; i < N; ++i) {
        v.PushBack(t[i], alloc);
