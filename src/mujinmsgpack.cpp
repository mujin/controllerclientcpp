        ...
        switch (o.type)
        {
            ...
            case msgpack::type::EXT: {
                if (o.via.ext.type() == -1) {
                    ...
                    MUJIN_LOG_WARN("Unrecognized msgpack extension type.");
                }
                break;
            }
...
void mujinmsgpack::DumpMsgPack(const rapidjson::Value& value, std::ostream& os)
{
    msgpack::osbuffer buf(os);
    msgpack::pack(buf, value);
}

void mujinmsgpack::DumpMsgPack(const rapidjson::Value& value, std::vector<char>& output)
{   
    msgpack::vbuffer buf(output);
    msgpack::pack(buf, value);
}

void mujinmsgpack::ParseMsgPack(rapidjson::Document& d, const std::string& str)
{
...
void mujinmsgpack::ParseMsgPack(rapidjson::Document& d, const void* data, size_t size)
{
    msgpack::unpacked unpacked;
...
void mujinmsgpack::ParseMsgPack(rapidjson::Document& d, std::istream& is)
{
    std::string str;
    str.assign(std::istreambuf_iterator<char>(is), std::istreambuf_iterator<char>());
    mujinmsgpack::ParseMsgPack(d, str);
}
