...
/** \example mujintransform.cpp
    ...
    example1: mujintransform --controller_hostname=yourhost
    example2: mujintransform --controller_hostname=yourhost
 */
...
bool ParseOptions(int argc, char ** argv, bpo::variables_map& opts)
{
    ...
    desc.add_options()
        ...
        ("unit", bpo::value<string>()->default_value("mm"), "length unit of transformation")
        ("targetname", bpo::value<string>(), "target to move")
        ("destination", bpo::value<vector<double> >()->multitoken(), "destination to move to, \'X Y Z RX RY RZ\'")
        ("relative", bpo::bool_switch(), "relative transformation from current ")
        ;
